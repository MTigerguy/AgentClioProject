...
def classify_and_split_pdf(input_path, log_callback):
    ...
    output_folder = os.path.join(documents_folder, "Mondays Files")
    os.makedirs(output_folder, exist_ok=True)

    original_name = os.path.splitext(os.path.basename(input_path))[0]
    today_str = datetime.now().strftime("%Y-%m-%d")

    with open(os.path.join(output_folder, f"LETTER_{original_name}_{today_str}.pdf"), "wb") as f:
        letter_writer.write(f)
    with open(os.path.join(output_folder, f"LEGAL_{original_name}_{today_str}.pdf"), "wb") as f:
        legal_writer.write(f)
    with open(os.path.join(output_folder, f"LETTER_IN_ORDER_{original_name}_{today_str}.pdf"), "wb") as f:
        letter_with_blanks.write(f)
    with open(os.path.join(output_folder, f"LEGAL_IN_ORDER_{original_name}_{today_str}.pdf"), "wb") as f:
        legal_with_blanks.write(f)
